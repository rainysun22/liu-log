...
***
## 1.6 特征对象
`特征对象指向实现了trait特征的类型的实例`
`可以通过 & 或 Box<T> 智能指针的方式来创建特征对象`
...
```
### 特征对象的动态分发
1. 泛型是在编译器完成处理的：编译器会为每一个泛型参数对应的具体类型生成一份代码，这叫做`静态分发`
2. 特征对象是在运行时完成处理的：运行时会根据对象的类型来调用对应的方法，这叫做`动态分发`。`dyn`关键字就是动态分发的意思。

### Self与self
```
// self指代当前实例对象，即button
// Self指代Button类型
trait Draw {
    fn draw(&self) -> Self;
}
